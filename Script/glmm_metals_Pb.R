...
ggsave("lead_boxplot.png", plot = last_plot(), units = "px", dpi = 450, path = "Figures/glmm_metals/Lead") 

# Fitting the model
mod_Pb <- lmer(log(Pb_conc) ~ legacy + (1|site_id), data = metal, REML = TRUE)

# Goodness-of-fit
r.squaredGLMM(mod_Pb)

# Test of effects: KR approximation
anova(mod_Pb, type = 3, ddf = "Kenward-Roger")

# Checking for violated model assumptions
check_model(mod_Pb)
simul_output <- (simulateResiduals(fittedModel = mod_Pb))
plot(simul_output) 
...
metal$predict_Pb <- predict(mod_Pb, type = "response", newdata = metal)
metal$predict_Pb <- exp(metal$predict_Pb) # Since model predictions are logarithmic
ggplot(metal, aes(x = legacy, y = predict_Pb, fill = legacy)) +
  geom_boxplot() +
  geom_jitter(shape = 16, size = 1.5, position = position_jitter(0.2)) + 
  theme_gray() +
  ...
  scale_x_discrete(labels = c("Forested","Agricultural","Industrial")) + 
  scale_fill_manual(values = c("forestgreen","gold","blue"))  
ggsave("glmm_result.png", plot = last_plot(), units = "px", dpi = 450, path = "Figures/glmm_metals/Lead")
